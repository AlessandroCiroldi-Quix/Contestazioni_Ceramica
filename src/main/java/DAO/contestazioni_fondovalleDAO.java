...
@ApplicationScoped
@Data
public class contestazioni_fondovalleDAO {
    @Inject
    Producer.jdbiProducer jdbiProducer;

    //* SELECT
    // Questa funzione restituisce i dati delle contestazioni italcer
    public List<contestazioni_fondovalleDTO> getDataFondovalle() {
        Jdbi jdbi = jdbiProducer.getJdbi();

        String query = "SELECT * FROM contestazioni_fondovalle";

        return jdbi.withHandle(handle -> handle.createQuery(query)
                .mapToBean(contestazioni_fondovalleDTO.class)
                .list()
        );
    }
    ...
    public void addContestazione_fondovalle(contestazioni_fondovalleDTO contestazione) {
        Jdbi jdbi = jdbiProducer.getJdbi(); // Otteniamo l'oggetto Jdbi dal produttore Jdbi

        String query = "INSERT INTO contestazioni_fondovalle VALUES (:id, :cod_cliente, :rs_cliente, :cod_articolo," +
    ...
    @SuppressWarnings("unused")
    public void deleteContestazione_fondovalle(contestazioni_fondovalleDTO contestazione) {
        Jdbi jdbi = jdbiProducer.getJdbi();  // Inizializza l'oggetto Jdbi utilizzando il jdbiProducer

        String query = "DELETE FROM contestazioni_fondovalle WHERE id = :id";  // Query per eliminare una riga dalla tabella 'contestazioni_italcer' basata sull'ID
    ...
    //* FIND
    // Metodo per trovare una contestazione
    public boolean findContestazione_fondovalle(contestazioni_fondovalleDTO contestazione) throws WebApplicationException {
        ...
        String query = "SELECT id FROM contestazioni_fondovalle WHERE id = :id";
    ...
    // Metodo per aggiornare un record nella tabella 'contestazioni_fondovalle'
    public void updateContestazioni_fondovalle(contestazioni_fondovalleDTO contestazione, String id) {
        Jdbi jdbi = jdbiProducer.getJdbi(); // Ottiene l'oggetto Jdbi tramite jdbiProducer

        String query = "UPDATE contestazioni_fondovalle SET cod_cliente = :cod_cliente, rs_cliente = :rs_cliente, " +
