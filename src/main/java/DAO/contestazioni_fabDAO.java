...
@ApplicationScoped
@Data
public class contestazioni_fabDAO {
    @Inject
    Producer.jdbiProducer jdbiProducer;

    //* SELECT
    // Questa funzione restituisce i dati delle contestazioni italcer
    public List<contestazioni_fabDTO> getDataFab() {
        Jdbi jdbi = jdbiProducer.getJdbi();

        String query = "SELECT * FROM contestazioni_fab";

        return jdbi.withHandle(handle -> handle.createQuery(query)
                .mapToBean(contestazioni_fabDTO.class)
                .list()
        );
    }
    ...
    public void addContestazione_fab(contestazioni_fabDTO contestazione) {
        Jdbi jdbi = jdbiProducer.getJdbi(); // Otteniamo l'oggetto Jdbi dal produttore Jdbi

        String query = "INSERT INTO contestazioni_fab VALUES (:id, :cod_cliente, :rs_cliente, :cod_articolo," +
    ...
    @SuppressWarnings("unused")
    public void deleteContestazione_fab(contestazioni_fabDTO contestazione) {
        Jdbi jdbi = jdbiProducer.getJdbi();  // Inizializza l'oggetto Jdbi utilizzando il jdbiProducer

        String query = "DELETE FROM contestazioni_fab WHERE id = :id";  // Query per eliminare una riga dalla tabella 'contestazioni_italcer' basata sull'ID
    ...
    public boolean findContestazione_fab(contestazioni_fabDTO contestazione) throws WebApplicationException {
        ...
        String query = "SELECT id FROM contestazioni_fab WHERE id = :id";
    ...
    public void updateContestazioni_fab(contestazioni_fabDTO contestazione, String id) {
        Jdbi jdbi = jdbiProducer.getJdbi(); // Ottiene l'oggetto Jdbi tramite jdbiProducer

        String query = "UPDATE contestazioni_fab SET cod_cliente = :cod_cliente, rs_cliente = :rs_cliente, " +
        ...
        if (rowsAffected != 1) {
            throw new WebApplicationException("Errore nell'aggiornamento della contestazione fab.");
        }
    }
}
